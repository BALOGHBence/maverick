...
def fetch_image_urls(query : str, max_links_to_fetch : int, wd : webdriver,
    ...
    while image_count < max_links_to_fetch:
        ...
        print(f"Found: {number_results} search results. Extracting links "
              "from {results_start}:{number_results}")
        ...
        for img in thumbnail_results[results_start:number_results]:
            ...
            for actual_image in actual_images:
                if actual_image.get_attribute('src') and 'http' \
                        in actual_image.get_attribute('src'):
            ...
            print("Found:", len(image_urls), "image links, "
                  "looking for more ...")
            time.sleep(30)
            return
...
def persist_image(folder_path : str, url : str):
    ...
    try:
        ...
        file_path = os.path.join(folder_path,hashlib.sha1(image_content).
                                 hexdigest()[:10] + '.jpg')
...
def search_and_download(search_term : str, driver_path : str,
                        target_path = './images', number_images=5):
    target_folder = os.path.join(target_path,'_'.
                                 join(search_term.lower().split(' ')))
...
# 91.0.4472.77
search_term = "Orban Viktor"
DRIVER_PATH = "E:\\chromedriver.exe"
FOLDER_PATH = "E:\\googlescrape"
search_and_download(search_term = search_term, driver_path = DRIVER_PATH,
                    target_path=FOLDER_PATH, number_images = 100)
