...
class TestMinimumRaiseTracking(unittest.TestCase):
    ...
    def test_last_raise_size_updated_on_raise(self):
        ...
        IMPORTANT: In heads-up, the button posts the small blind.
        Your engine semantics use "raise-by (chips added now)" which includes the call portion.
        ...
        # Heads-up: button (p1) is SB and acts first preflop
        game._take_action_from_current_player()

        # last_raise_size should be the raise increment (40) and table bet should be 60
...
class TestNonReopeningAllIn(unittest.TestCase):
    ...
    def test_short_all_in_does_not_reopen_betting(self):
        ...
        game.add_player(p1, state=PlayerState(stack=1000))
        game.add_player(p2, state=PlayerState(stack=1000))
        game.add_player(p3, state=PlayerState(stack=30))
        ...
        # Button starts at index 0 by default; blinds:
        # P2 has SB=10, P3 has BB=20 (leaving 10 chips)
    ...
    def test_all_in_meeting_min_raise_reopens_betting(self):
        ...
        game.add_player(p1, state=PlayerState(stack=1000))
        game.add_player(p2, state=PlayerState(stack=1000))
        game.add_player(p3, state=PlayerState(stack=40))
...
class TestRaiseBySemantics(unittest.TestCase):
    ...
    def test_raise_amount_is_added_to_current_bet(self):
        """
        Test that RAISE.amount is added to player's current bet (chips added now).
...
class TestMinimumRaiseEnforcement(unittest.TestCase):
    ...
    def test_all_in_available_when_cant_min_raise(self):
        ...
        valid = game._get_valid_actions(game.get_player_snapshot(p1.uid))
...
class TestReopenLogicWithZeroRaise(unittest.TestCase):
    ...
    def test_zero_last_raise_size_does_not_reopen_on_zero_raise(self):
        """Documentation test for reopen guard logic."""
        game = Game(small_blind=10, big_blind=20, max_hands=1)
