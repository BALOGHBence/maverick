...
class SharkBot(Player):
    ...
    def decide_action(
        ...
        if ActionType.RAISE in valid_actions:
            ...
            raise_amount = min(
                max(min_raise, int(game_state.pot * 0.75)), self.state.stack
            )
            return PlayerAction(
        ...
        if ActionType.BET in valid_actions:
            ...
            bet_amount = min(int(game_state.pot * 0.8), self.state.stack)
            if bet_amount < game_state.big_blind:
        ...
        if ActionType.CALL in valid_actions:
            ...
            if call_amount <= self.state.stack and call_amount <= game_state.pot * 0.66:
                return PlayerAction(
